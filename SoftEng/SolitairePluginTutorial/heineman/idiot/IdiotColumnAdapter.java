 ...
 */
public IdiotColumnAdapter (TestColumnDrag theGame, ColumnView columnView) {
...
public void mousePressed (java.awt.event.MouseEvent me) {
	...
	ColumnView cv = src.getColumnView(me);
...
public void mouseReleased (java.awt.event.MouseEvent me) {
	...
	Widget w = c.getActiveDraggingObject();
	if (w == c.getNothingBeingDragged()) return;
	
	/** Must be the CardView widget. */
	ColumnView cv = (ColumnView) w;
	Column theCol = (Column) cv.getModelElement();
	if (theCol == null) {
	...
	Move m = new MoveCardsMove (fromColumn, theCol, toColumn);
	if (m.doMove (theGame)) {
		// Successful move!  
	...
	theGame.repaintAll (cv.getBounds());   // first refresh the last known position of the card being dragged.
