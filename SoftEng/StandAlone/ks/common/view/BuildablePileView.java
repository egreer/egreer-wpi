...
public class BuildablePileView extends Widget {
	...
	/** Keep track of number of facedown cards. Used to draw properly. */
...
public ColumnView getColumnView(java.awt.event.MouseEvent me) {
	...
	if (numFaceUp == 0) return null; // no chance for returning ColumnView if no faceup cards.
	
	java.awt.Rectangle r = new java.awt.Rectangle (0, deltaY, cards.getWidth(), deltaY + cards.getHeight() + (numFaceUp-1) * cards.getOverlap());
	if (r.contains (p) == false) return null;  // wasn't on a faceup Card.
...
public void redraw() {
	...
	g.setColor (oldColor);  // restore graphicsContext color
