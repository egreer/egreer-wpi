...
// perhaps move this logic back into container?
...
abstract public class Solitaire {
	...
	/** seed to use for this game. */
	protected int seed;

	/** The increment to add to seed when starting a new hand. */
	...
	public CardImages getCardImages() {
		CardImages ci = CardImagesLoader.getDeck(getContainer(), getDeckType());
		return ci;
	}
	 ...
	 * 
	 * @return int increment to be used to update random seed for successive
	 *         games.
	 */
	 ...
	 * 
	 * @return int represents the seed used for randomization algorithms
	 */
	public int getSeed() {
		return seed;
	}
	...
	/**
	 * Refresh all elements whose Widgets are now dirty. Also redraw background.
	 * Not all Widgets are redrawn.
	 */
	 ...
	 * redrawn.
	 * <p>
	 * Clip to only refresh widgets whose view intersects the clip rectangle.
	 * [clip is unaffected by this invocation. ]
	 * 
	 * @param clip
	 *            a Rectangle representing the region outside of which no
	 *            drawing occurs.
	 */
	public void refreshWidgets(Rectangle clip) {
		container.refreshWidgets(clip);
	}
	 ...
	 * <p>
	 * Note that if you call container.repaint() the entire background is
	 * re-filled with the background color, introducing flicker. The
	 * repaintAll() method is our own protocol that avoids this flicker.
	 */
	 ...
	 * Forces entire game to be repainted, whether a View needs it or not.
	 * <p>
	 * Restrict drawing to the clip Rectangle. [clip is unaffected by this
	 * invocation].
	 * <p>
	 * Note that if you call container.repaint() the entire background is
	 * re-filled with the background color, introducing flicker. The
	 ...
	 *            drawing occurs.
	 */
	public void repaintAll(Rectangle clip) {
		container.repaintAll(clip);
	}
	 ...
	 *            drawing occurs.
	 */
	public void repaintWidgets(Rectangle clip) {
		container.repaintWidgets(clip);
	}
	 ...
	 *            when generating successive solitaire deals.
	 */
	 ...
	 *            drawing occurs.
	 */
	public void repaintBackground(Rectangle clip) {
		container.repaintBackground(clip);
	}
}