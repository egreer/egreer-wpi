  ...
  if ((net = bpnn_read(netname)) == NULL) {
	...
	make a net with:
	  imgsize inputs, 4 hiden units, and 1 output unit
          */
      net = bpnn_create(imgsize, 4, 2); //CHANGE
    } else {
...
double *err;
{
  ...
  delta = (net->target[1] - net->output_units[1]); + (net->target[2] - net->output_units[2]);
