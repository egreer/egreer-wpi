...
public class RemoveIncorrectListenerTest extends TestCase {
	...
	public void testActionPerformed() {
		...
		IdIncorrectListener listener1 = new IdIncorrectListener(gui);
		listener1.actionPerformed(new ActionEvent(this, 1, ""));
		for (Cell cell : gui.getPuzzle().getIdentifiedIncorrectCells()) {
			locs.add(cell.loc);
		}

		RemoveIncorrectListener listener = new RemoveIncorrectListener(gui);
		listener.actionPerformed(new ActionEvent(this, 1, ""));

		for (Iterator<Location> it = locs.iterator(); it.hasNext(); ){
			
			assertNull(gui.getPuzzle().getPlayerGrid().getCellAtLocation(it.next()).getDigit()); 
		}
	}
}
