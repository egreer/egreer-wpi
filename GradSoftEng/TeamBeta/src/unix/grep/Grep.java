...
/**
 * Process Input based upon regular expressions. CountOnly True/False (-c
 * option) CaseSensitive True/False (-i option) Negative True/False (-v option)
 * Expression e (-e option)
 */
@Provide( { IOutput.class })
@Require( { IOutput.class })
@Property( { Grep.propCountOnly, Grep.propNegative, Grep.propCaseSensitive,
		Grep.propExpression })
public class Grep extends ThreadedComponent implements ICustomizableComponent {
	...
	public void customize(Properties props) throws Exception {
		...
		if (s != null) {
			countOnly = Boolean.valueOf(s);
		}

		s = props.getProperty(propNegative);
		if (s != null) {
			negative = Boolean.valueOf(s);
		}

		s = props.getProperty(propCaseSensitive);
		if (s != null) {
			caseSensitive = Boolean.valueOf(s);
		}
