...
public class RowView extends Widget implements ElementListener {
	...
	/** Image maintained by this widget to perform any drawings. */
 ...
 * This may be overriden by subclasses, to return any number from 0 to count()-1.
 */
...
public RowView getRowView (java.awt.event.MouseEvent me) {
	...
	if (topR.contains (p)) {
		...
		// all that matters is the direction within covered Rectangle.
		if (direction == LEFT) {
			numSelectedCards = 2 + (p.x - coveredCardsR.x)/cards.getOverlap();
			newBounds.width += (numSelectedCards-1)*cards.getOverlap();
		} else {
			numSelectedCards = 2 + (coveredCardsR.x + coveredCardsR.width - p.x)/cards.getOverlap();
			newBounds.width += (numSelectedCards-1)*cards.getOverlap();
			newBounds.x -= 	(numSelectedCards-1)*cards.getOverlap();
		}
	}
...
public void redraw() {
	...
	java.awt.Color oldColor = g.getColor();     // store old color for restoration later
