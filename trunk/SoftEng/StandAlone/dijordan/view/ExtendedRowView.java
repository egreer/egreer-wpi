...
public class ExtendedRowView extends RowView {
  ...
  /*****************************************************************
   * Returns the top card, without removing it, if it was clicked on
   * much of this code was copied from RowView.getCardViewForTopCard
   */
  public Card returnTopCard(java.awt.event.MouseEvent me) {
	Column col = (Column) getModelElement();
	...
	// Make sure to adjust for the beginning drawing (firstCard()).  Was NumCards - 1
	int deltaX = numCoveredCards() * cards.getOverlap();
	java.awt.Rectangle r;
	if (direction == RIGHT) {
	  r = new java.awt.Rectangle (deltaX, 0, cards.getWidth(), cards.getHeight());
	} else {
	  r = new java.awt.Rectangle (0, 0, cards.getWidth(), cards.getHeight());
	}
	if (r.contains (p) == false) {
	  // wasn't on the top card, so return.
	  return null;
	}
	return(col.peek());
  }      
