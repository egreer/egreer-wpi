...
public class Prb5 {
	...
	public static void main(String args[]){
		...
		for (ShuttleDataInstance test : testInstances){
			...
			for (ShuttleDataInstance m : minimum){
				if (m.classification.equals("auto")) countAuto += 1.0/m.distance;
				else  if(m.classification.equals("noauto")) countNo += 1.0/m.distance;
	...
	static double euclideanDistance(ShuttleDataInstance training, ShuttleDataInstance test){
		...
		distance += Math.sqrt(2*Math.pow((training.stability - test.stability), 2) + 1*Math.pow((training.error - test.error), 2));
		if (training.headWind != test.headWind) distance += 4.5; //Head wind
		if (training.visability != test.visability) distance += 3; //Visability
